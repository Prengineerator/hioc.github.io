...
- **Tagging:** `POST /api/owner/suggest/traits/generate` (owner-only) sends the whole menu (name, description, category, parent) to whichever provider `deciderProvider()` selects, with the trait fields as structured output. It upserts only rows that are missing or unconfirmed (**never overwrites `confirmed` rows**).
  - **Opus / Gemini Flash:** one JSON-schema call per batch. Anthropic batches of 40 run ~3-at-a-time; Gemini batches of 15 run through a concurrency-4 pool, all sharing one 50s budget (small batches + concurrency, rather than one call at a time, is what keeps this fast on the free tier).
  - **Jev (preferred when `TYPESAFE_API_KEY` is set):** Jev is decision-only and can't write a batch of JSON rows, so it tags **one item per `systemOne` call**, through a concurrency-8 pool sharing one 50s budget. Every trait field is its own question: `temperature`/`caffeine`/`body`/`kind` are `choice` questions; `is_coffee` is a `noul` (kept `true` when P(yes) ≥ 0.5); `sweetness` is a 4-level `score` (`Math.round(expected score)`, clamped 0–3); each of the six moods and four dayparts gets its own one-line `noul` question (kept when ≥ 0.5; if none clear the mood bar the single highest-probability mood is kept anyway, and if none clear the daypart bar all four are kept — an item is never taggable into zero moods or zero dayparts). Jev can't write free text either, so `flavor_notes` comes from a **fixed 20-word vocabulary** (`JEV_FLAVOR_VOCABULARY` in `lib/suggest/traitsPrompt.ts`) — one `noul` per word, kept when ≥ 0.6, highest-probability first, capped at 5. Every provider's output — Jev included — is assembled into the same row shape and run through the same `validateOpusTraitRows()` allow-lists (playbook S-2) before it's trusted.
  - **Low-confidence review hints (Jev only, no DB column):** for each item, if the `temperature`/`caffeine`/`kind` choice confidence is < 0.6, or the `is_coffee` noul lands in the uncertain 0.35–0.65 band, that item's name is added to the generate response's `needsReview: string[]`. The Traits tab shows "Jev was unsure about these — please check them first: …" so the owner reviews exactly those rows first, without any extra state to reconcile.
  - **Partial-success reporting:** the response's `error` field is set whenever ANY item/batch failed — not only when nothing got tagged — so "38 of 40 tagged" still tells the owner what went wrong with the other 2. A Gemini free-tier rate limit (HTTP 429) stops the pool from starting further batches (rows already tagged are kept) and reports a friendly, actionable message rather than the raw HTTP error.
