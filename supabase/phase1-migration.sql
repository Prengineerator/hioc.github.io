-- HIOC Phase 1 "Connected Ordering" — schema migration.
-- Companion to docs/PHASE-1-SPEC.md §5. ADDITIVE and (where possible)
-- idempotent — safe to run on top of an existing schema.sql database.
--
...
--   transaction it was added in — so the value additions must be committed
--   before the tables/policies/views below reference them. Then run the rest.
--
-- WHEN APPLIED FOR REAL: update lib/types.ts in the SAME PR to mirror every
-- change here (existing repo convention — the two must never drift), and add
-- the matching RLS review + tests noted in the Phase-1 DoD.

-- ===========================================================================
-- SECTION 1 — Enum extensions  (RUN THIS BLOCK FIRST, THEN COMMIT)
-- ===========================================================================

-- Fulfillment lifecycle (docs/REVAMP-REQUIREMENTS.md §5). Existing values:
-- 'received','preparing','ready','completed'. Add the accept/reject/cancel
-- states + a forward-compat 'placed' (used later when online payment gates
-- an order before it is 'received'). Default stays 'received'.
