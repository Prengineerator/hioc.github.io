...
// guest or table-QR order must never be offered — or told about — "pay at
// counter".
...
describe('canPayAtCounter (issue-1)', () => {
  ...
  it('is false for a table-QR order, even with a logged-in user_id', () => {
    expect(canPayAtCounter({ channel: 'table_qr', user_id: null })).toBe(false);
    expect(canPayAtCounter({ channel: 'table_qr', user_id: 'cust-1' })).toBe(false);
  });
...
describe('paymentFlagMessage (issue-1 wording)', () => {
  ...
  it('never mentions the counter for a guest/table-QR order', () => {
