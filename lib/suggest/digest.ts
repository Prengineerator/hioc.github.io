...
// (Sonnet, per §1's routing rule: this is execution — summarising numbers
// already computed — not a decision). Only AGGREGATES (SuggestionStats) are
// ever sent; there is no per-customer field on that type to leak (playbook
// S-3). On any failure this falls back to a plain template built from the
// same stats, so the digest row is never missing and never blocks the cron.
//
// 'server-only' — reads ANTHROPIC_API_KEY via getAnthropicClient() (S-1).

import 'server-only';
import { getAnthropicClient } from './anthropic';
import { costUsdMicros, workerModel } from './models';
...
/**
 * Produces the weekly digest text for `stats`. Always resolves — a missing
 * key, a timeout, a non-`end_turn` stop, or a summary that breaks the word/
 * bullet rules all fall back to the template rather than throwing, so the
 * cron route (SUG-12 AC: "given the LLM fails, a digest row with the plain
 * numbers and source='template'") always has something to store.
 */
